...
if (
    ARDUINO_ESP32_PATH
    and (Path(ARDUINO_ESP32_PATH) / "tools" / "esp32-arduino-libs").is_dir()
):
    SDKCONFIG_DIR = Path(ARDUINO_ESP32_PATH) / "tools" / "esp32-arduino-libs"
elif (
    GITHUB_WORKSPACE
    and (Path(GITHUB_WORKSPACE) / "tools" / "esp32-arduino-libs").is_dir()
):
    SDKCONFIG_DIR = Path(GITHUB_WORKSPACE) / "tools" / "esp32-arduino-libs"
else:
    raise EnvironmentError(
        "Could not locate esp32-arduino-libs directory. "
    ...
    if not args.arduino_cli_path or not args.user_path:
        ...
        if not args.arduino_cli_path:
            if arduino_ide_path:
                ...
                print(
                    "ERROR: Arduino CLI path not found in ARDUINO_IDE_PATH env var and not provided (-ai option)"
                )
        ...
        if not args.user_path:
            if arduino_usr_path:
                ...
                print(
                    "ERROR: Arduino user path not found in ARDUINO_USR_PATH env var and not provided (-au option)"
                )
                print("Run install-arduino-cli.sh first to set environment variables")
                sys.exit(1)
...
def build_example_for_target(sketch_dir, target, relative_path, args):
    ...
    if not check_requirements(str(sketch_dir), sdkconfig):
        print(
            f"Target {target} does not meet the requirements for {Path(sketch_dir).name}. Skipping."
        )
        return True
    ...
    if res.returncode == 0:
        ...
        if args.generate_diagrams:
            ...
            try:
                ...
                print(
                    f"WARNING: Failed to generate diagram for {relative_path} ({target}): {e}"
                )
    else:
    ...
    for example in examples:
        ...
        if args.generate_launchpad_config:
            ...
            try:
                ...
                print(
                    f"WARNING: Failed to generate LaunchPad config for {relative_path}/{target}: {e}"
                )
